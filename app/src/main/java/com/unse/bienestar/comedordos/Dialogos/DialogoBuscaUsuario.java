...
public class DialogoBuscaUsuario extends DialogFragment {
    ...
    private void loadListener() {
        btnOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (dni != 0) {
                    ...
                    ABC.showToast(getContextDialog(), getContextDialog().getString(R.string.primeroBuscar));
                }
            }
        });
        btnBuscar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                } else ABC.showToast(getContextDialog(), getString(R.string.campoVacio));
            }
        });
    ...
    private void reservar(int dni) {
        ...
        String URL = String.format("%s?idU=%s&key=%s&i=%s&im=%s&t=%s&ie=%s",
                ABC.URL_RESERVA_INSERTAR_ALUMNO, id, key, dni,
                getIdMenu(), 4, id);
    ...
    private void buscar(String dni) {
        ...
        String key = manager.getValueString(ABC.TOKEN);
        int id = manager.getValueInt(ABC.MY_ID);
        String URL = String.format("%s?id=%s&key=%s&idU=%s", ABC.URL_USUARIO_BY_ID_REDUCE,
                id, key, dni);
        StringRequest request = new StringRequest(Request.Method.GET, URL, new Response.Listener<String>() {
            ...
            public void onErrorResponse(VolleyError error) {
                error.printStackTrace();
                ABC.showToast(getContextDialog(), getString(R.string.servidorOff));
                dialog.dismiss();
    ...
    private void procesarRespuesta(String response) {
        try {
            ...
            switch (estado) {
                case -1:
                    ABC.showToast(getContextDialog(), getString(R.string.errorInternoAdmin));
                    break;
                case 1:
                    //Exito
                    loadInfo(jsonObject);
                    break;
                case 2:
                    ABC.showToast(getContextDialog(), getString(R.string.usuarioNoExiste));
                    break;
                case 3:
                    ABC.showToast(getContextDialog(), getString(R.string.tokenInvalido));
                    break;
                case 4:
                    ABC.showToast(getContextDialog(), getString(R.string.camposInvalidos));
                    break;
                case 100:
                    //No autorizado
                    ABC.showToast(getContextDialog(), getString(R.string.tokenInexistente));
                    break;
            }
