...
public class PDFMonthActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void loadIntent() {
        if (getIntent().getSerializableExtra(ABC.ID_MENU) != null) {
            menus = (ArrayList<Menu>) getIntent().getSerializableExtra(ABC.ID_MENU);
        }
        if (getIntent().getSerializableExtra(ABC.ALUMNO_NAME) != null) {
            alumnos = (ArrayList<Usuario>) getIntent().getSerializableExtra(ABC.ALUMNO_NAME);
        }
        if (getIntent().getSerializableExtra(ABC.RESERVA) != null) {
            reservas = (ArrayList<Reserva>) getIntent().getSerializableExtra(ABC.RESERVA);
        }
        if (getIntent().getStringExtra(ABC.MESNAME) != null) {
            mes = getIntent().getStringExtra(ABC.MESNAME);
        }
        if (getIntent().getIntExtra(ABC.NUMERO_MES, -1) != -1) {
            numberMes = getIntent().getIntExtra(ABC.NUMERO_MES, -1);
        }
    }
    ...
    private void loadEstadisticasFacultad() {
        ...
        for (Integer integer : facultades) {
            yEntry.add(new PieEntry(integer, ABC.facultad[i]));
            i++;
        }
    ...
    private void generatePDF() {
        Bitmap graficoBarra = ABC.getBitmapFromView(findViewById(R.id.cardReservas));
        Bitmap graficoTorta = ABC.getBitmapFromView(findViewById(R.id.cardPorFacultad));
