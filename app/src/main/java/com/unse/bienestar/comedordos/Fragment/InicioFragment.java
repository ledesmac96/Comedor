...
import com.unse.bienestar.comedordos.Activity.NuevaReservaEspecialActivity;
import com.unse.bienestar.comedordos.Database.MenuViewModel;
import com.unse.bienestar.comedordos.Database.ReservaViewModel;
import com.unse.bienestar.comedordos.Database.RolViewModel;
import com.unse.bienestar.comedordos.Dialogos.DialogoBuscaUsuario;
...
public class InicioFragment extends Fragment implements View.OnClickListener {
    ...
    private void loadInfo() {
        ...
        String URL = String.format("%s?idU=%s&key=%s&d=%s", ABC.URL_MENU_HOY, id, key, 1);
    ...
    private void loadInternal() {
        int id = mPreferenciasManager.getValueInt(ABC.ID_MENU);
        if (id != 0) {
    ...
    private void loadMenu(Menu menu) {
        if (menu != null) {
            ...
            mMenu = menu;
            if ((isAdmin || mMenu.getDisponible() == 1) && ABC.isShowByHour(isAdmin ? 27 : 24)) {
                loadInfoCardView(menu);
            } else if (isAdmin && ABC.isShowByHour(27)) {
                loadInfoCardView(menu);
            } else {
                cardNo.setVisibility(View.VISIBLE);
    ...
    private void loadInfoCardView(Menu menu) {
        ...
        mPreferenciasManager.setValue(ABC.ID_MENU, mMenu.getIdMenu());

        txtFecha.setText(ABC.getDate(menu.getDia(), menu.getMes(), menu.getAnio()));
        String[] comida = ABC.getComidas(menu.getDescripcion());
        String food = String.format("%s - %s", comida[0], comida[1]);
        txtPlato.setText(food.length() > 60 ? food.substring(0, 59) + "..." : food);
    ...
    private void reservaEspecial() {
        ...
        intent.putExtra(ABC.ID_MENU, mMenu != null ? mMenu.getIdMenu() : -1);
    ...
    private void restringir() {
        ...
        String key = manager.getValueString(ABC.TOKEN);
        int id = manager.getValueInt(ABC.MY_ID);
        String URL = String.format("%s?idU=%s&key=%s&idM=%s&val=%s", ABC.URL_MENU_RESTRINGIR,
    ...
    private void reservarMenu() {
        ...
        String key = manager.getValueString(ABC.TOKEN);
        int id = manager.getValueInt(ABC.MY_ID);
        String URL = String.format("%s?idU=%s&key=%s&i=%s&im=%s&t=%s", ABC.URL_RESERVA_INSERTAR, id, key, id,
                mMenu.getIdMenu(), 4);
        ...
        try {
            ...
            switch (estado) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    ABC.showToast(getContext(), getString(R.string.reservaNoExiste));
                    dialogReserva(false, null);
                    break;
                case 3:
                    ABC.showToast(getContext(), getString(R.string.tokenInvalido));
    ...
    private void terminar() {
        ...
        String key = manager.getValueString(ABC.TOKEN);
        int id = manager.getValueInt(ABC.MY_ID);
        String URL = String.format("%s?idU=%s&key=%s&idM=%s&val=%s", ABC.URL_MENU_TERMINAR,
